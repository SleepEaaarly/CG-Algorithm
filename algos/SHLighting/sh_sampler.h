...
class SHSampler {
  private:
    ...
    bool read_sh_coeffs() {
        ...
        std::string header = ResourceManager::getInstance()
                                 .getTexture2D("hdr_texture")
                                 ->getPath();
  ...
  public:
    SHSampler(size_t deg, const char *txt_path, bool load = true)
        : degree(deg), txt_path(txt_path) {
        if (load)
            has_data = read_sh_coeffs();
        else
            has_data = false;
    }
    ...
    void calcSHCoeffs(std::shared_ptr<Cubemap> cubemap, bool save,
                      int com = 3) {
        // com = 3 : rgb
            ...
            for (auto row = 0; row < height; ++row) {
                for (auto col = 0; col < width; ++col) {
                    ...
                    glm::vec3 color(image_data[ind], image_data[ind + 1],
                                    image_data[ind + 2]);
                    auto world_vec =
                        getTexelWorldVector(face, row, col, height, width);
                    auto dw = solidAnglePixel(col, row, width);
                    for (int l = 0; l <= degree; ++l) {
                        for (int m = -l; m <= l; ++m) {
                            int ind = l * (l + 1) + m;
                            sh_coeffs[ind] +=
                                color * getBasis(world_vec, l, m) * dw;
                        }
                    }
                }
            }
    ...
    float solidAnglePixel(int u, int v, int N) {
        ...
        return std::atan2(x1 * y1, R(x1, y1)) - std::atan2(x0 * y1, R(x0, y1)) -
               std::atan2(x1 * y0, R(x1, y0)) + std::atan2(x0 * y0, R(x0, y0));
    }

    glm::vec3 getTexelWorldVector(int face, unsigned int row, unsigned int col,
    ...
    float getBasis(const glm::vec3 &world_vec, int l, int m) {
        ...
        if (l == 0) {
            ...
            const float c1 =
                0.457046; // 0.25 * std::sqrt(21.0/2.0) * inv_sqrt_pi
            const float c2 = 1.445306; // 0.25 * std::sqrt(105) * inv_sqrt_pi
            const float c3 =
                0.590044; // 0.25 * std::sqrt(35.0/2.0) * inv_sqrt_pi
            switch (m) {
            case 0:
                return c0 * z * (2 * z * z - 3 * x * x - 3 * y * y);
            case 1:
                return c1 * x * (5 * z * z - 1);
            case -1:
                return c1 * y * (5 * z * z - 1);
            ...
            case -2:
                return 2 * c2 * x * y * z;
            case 3:
                return c3 * x * (x * x - 3 * y * y);
            case -3:
                return c3 * y * (3 * x * x - y * y);
            }
        } else {
    ...
    const std::vector<glm::vec3> getSHLightCoeffs() { return sh_coeffs; }
